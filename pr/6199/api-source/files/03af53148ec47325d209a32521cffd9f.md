...
-   对应不带SortConfig的函数原型
    ...
    **表2**  参数说明
    ...
    | repeatTime | 输入 | 重复迭代次数，int32_t类型。<!-- npu="950" id17 --><br>    Ascend 950PR&950DT系列产品：每次迭代完成32个元素的排序，下次迭代concat和index各跳过32个elements，dst跳过32*8 Byte空间。取值范围：repeatTime∈[0,255]。<!-- end id17 --><!-- npu="A3" id18 --><br>    Atlas A3系列产品：每次迭代完成32个元素的排序，下次迭代concat和index各跳过32个elements，dst跳过32*8 Byte空间。取值范围：repeatTime∈[0,255]。<!-- end id18 --><!-- npu="910b" id19 --><br> Atlas A2系列产品：每次迭代完成32个元素的排序，下次迭代concat和index各跳过32个elements，dst跳过32*8 Byte空间。取值范围：repeatTime∈[0,255]。<!-- end id19 --><!-- npu="310p" id20 --><br>Atlas推理系列产品AI Core：每次迭代完成16个region proposal的排序，下次迭代concat和dst各跳过16个region proposal。取值范围：repeatTime∈[0,255]。<!-- end id20 --><!-- npu="x90" id7 --><br>Kirin X90：每次迭代完成32个元素的排序，下次迭代concat和index各跳过32个elements，dst跳过32*8 Byte空间。取值范围：repeatTime∈[0,255]。<!-- end id7 --><!-- npu="9030" id8 --><br>Kirin 9030：每次迭代完成32个元素的排序，下次迭代concat和index各跳过32个elements，dst跳过32*8 Byte空间。取值范围：repeatTime∈[0,255]。<!-- end id8 --> |
