...
<!-- npu="950" id7 -->
针对Ascend 950PR&950DT系列产品，本接口不支持Subnormal场景：本接口内部实现使用到了除法运算符，由于除法运算符不支持Subnormal场景，当输入x为Subnormal数据时，会导致本接口最终结果为±inf；当计算结果处于Subnormal范围（例如x为较大正常数）时，输出会被刷新为保留符号的0。
<!-- end id7 -->
