...
CANN 9.0.0首次面向Ascend 950PR&950DT系列产品（3510架构）提供试验性C语言扩展层单指令多数据（Single Instruction Multiple Data，SIMD）API。基于指针编程，提供C完备编程能力，通过声明静态数组管理本地内存，由开发者自主管理同步与内存，适配C语言开发习惯，开放全部底层硬件能力，支撑精细化调优与极致性能实现。接口覆盖数据结构、数据搬运、标量操作、矩阵计算、同步控制、系统变量、缓存控制、原子操作、其他操作、Reg数据搬运和Reg矢量计算等能力。
