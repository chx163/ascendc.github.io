...
| 参数名 | 输入/输出 | 描述 |
| --- | --- | --- |
| `src` | 输入 | 待右移的源操作数。`src.mask`用于标记参与计算的元素，对应位置为`1`时参与计算，为`0`时不参与计算。 |
...
返回`reg_tensor<T>`。`reg`保存逐元素右移结果，`src.mask`对应位置为`0`时，`reg`的对应位置置零。返回值的`mask`与`src.mask`相同。

## 约束说明

- src.mask需通过`with_mask`接口预先设置。未设置时，mask的内容不确定，会导致参与计算的元素位置错误。
- 移位量为负数时行为未定义。移位量大于`src`元素类型位宽时，无符号输入返回`0`，
  有符号负数返回`-1`，有符号非负数返回`0`。
- 有符号整数在有效移位范围内使用符号位填充高位，无符号整数使用`0`填充高位。
...
__simd_vf__ inline void shift_right_example(InputTensor input, OutputTensor output)
{
