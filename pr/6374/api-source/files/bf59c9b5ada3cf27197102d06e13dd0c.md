...
该接口根据输入reg_tensor携带的mask，对源操作数按元素计算以10为底的对数，并返回计算结果。计算公式如下：
...
返回按元素计算以10为底对数的结果，类型为reg_tensor&lt;T&gt;。返回值mask与src.mask相同；src.mask对应位置为0时，返回值的对应元素置零。
...
__simd_vf__ inline void log10_example(InputTensor input, OutputTensor output)
{
