...
返回`reg_tensor<T>`。`reg`保存逐元素按位异或结果，`src0.mask`对应位置为`0`时，`reg`的对应位置置零。返回值的`mask`与`src0.mask`相同。
...
__simd_vf__ inline void bit_xor_example(
